...
class _EditSession {
  ...
  List<String> get _changes {
    var changes = <String>[];
    if (!isEqual || _swapChanges.isNotEmpty) {
      for (var dir in _indirectChanges.entries) {
        var dirChanges = _indirectChanges[dir.key];
        if (dirChanges != null) {
          for (var ch in dirChanges.entries) {
            if (ch.value.isNotEmpty && !changes.contains(ch.key)) {
              changes.add(ch.key);
            }
          }
        }
      }
      // Include swap-affected items for persistence
      for (var swapId in _swapChanges.keys) {
        if (!changes.contains(swapId)) {
          changes.add(swapId);
        }
      }
      if (!changes.contains(editing.id)) {
