...
class _ResizeMoveResult {
  ...
  /// IDs of items affected by a swap (need widget rebuild in the stack).
...
class _ItemCurrentLayout extends ChangeNotifier implements ItemLayout {
  ...
  /// Whether this item is currently a swap target (for visual highlight).
  final ValueNotifier<bool> _isSwapTarget = ValueNotifier<bool>(false);
  ...
  String? _currentSwapTarget;
  Offset? _swapDif;

  /// Undo any active swap, restoring all swapped items to their pre-swap
  /// positions. Returns the reverse pixel offset if a swap was active
  /// (so [_moveStartOffset] can be corrected), or null.
  Offset? _removeSwapAndGetReverseDif() {
    if (_swapping) {
      final reverseDif = _swapDif != null ? -_swapDif! : Offset.zero;

      // Clear visual highlight on the swap target
      if (_currentSwapTarget != null) {
        _layoutController._layouts![_currentSwapTarget]?._isSwapTarget.value =
            false;
      }

      // Restore every swapped item (including self) to pre-swap position
      _layoutController.editSession!._swapChanges.forEach((key, value) {
        _layoutController._layouts![key]!._backSwap(value);
        _layoutController._layouts![key]!.save();
      });
      _layoutController.editSession!._swapChanges.clear();
  ...
  /// Legacy wrapper – calls [_removeSwapAndGetReverseDif] but ignores result.
  void _removeSwap() {
    _removeSwapAndGetReverseDif();
  }

  /// Attempt to swap the dragged item (self) with [targetId]/[target].
  /// Returns a result if the swap succeeded, null otherwise.
  _ResizeMoveResult? _trySwap(
      String targetId, _ItemCurrentLayout target, Offset newTransform) {
    final editSession = _layoutController.editSession;
    if (editSession == null) return null;

    final aOrigin = editSession.editingOrigin;

    // Save A's pre-swap layout (current drag position)
    final aPreSwap = ItemLayout(
        startX: startX,
        startY: startY,
        width: width,
        height: height,
    ...
    // Save B's current layout
    final bPreSwap = ItemLayout(
    ...
    // --- Temporarily remove both from the grid ---
    _layoutController._removeFromIndexes(aPreSwap, id);
    _layoutController._removeFromIndexes(bPreSwap, targetId);

    // Try to place A at B's position
    final aAtB = _layoutController.tryMount(
        _layoutController.getIndex([bPreSwap.startX, bPreSwap.startY]),
        ItemLayout(
            startX: bPreSwap.startX,
            startY: bPreSwap.startY,
            width: _originSize![0],
            height: _originSize![1],
            minWidth: minWidth,
            minHeight: minHeight,
    ...
    if (aAtB == null) {
      // A can't fit at B's position – restore both, abort
      _layoutController._indexItem(aPreSwap, id);
      _layoutController._indexItem(bPreSwap, targetId);
      return null;
    }

    // Place A at B's position
    final xDif = aAtB.startX - startX;
    final yDif = aAtB.startY - startY;
    _startX = aAtB.startX;
    _startY = aAtB.startY;
    _width = aAtB.width;
    _height = aAtB.height;
    ...
    // Try to place B at A's editing-origin position (where it was at drag start)
    final bAtA = _layoutController.tryMount(
        _layoutController.getIndex([aOrigin.startX, aOrigin.startY]),
        ItemLayout(
            startX: aOrigin.startX,
            startY: aOrigin.startY,
            width: bPreSwap.width,
            height: bPreSwap.height,
            minWidth: target.minWidth,
            minHeight: target.minHeight,
            maxWidth: target.maxWidth,
            maxHeight: target.maxHeight));
    ...
    if (bAtA != null) {
      // Direct swap – B fits at A's origin
      target._startX = bAtA.startX;
      target._startY = bAtA.startY;
      target._width = bAtA.width;
      target._height = bAtA.height;
      target.save();
    } else {
      // Auto-relocate B to the nearest available space
      _layoutController.mountToTop(targetId, 0);
    }

    // Record swap for undo & persistence
    final dif = Offset(xDif * _slotEdge, yDif * _verticalSlotEdge);
    _swapping = true;
    _currentSwapTarget = targetId;
    _swapDif = dif;

    editSession._addSwap(
        id,
        targetId,
        _Swap(origin: aPreSwap),
        _Swap(origin: bPreSwap));

    // Visual highlight on target
    target._isSwapTarget.value = true;

    // Update drag transform so item keeps following the cursor
    if (_transform == null) {
      _transform = ValueNotifier(newTransform - dif);
      notifyListeners();
    } else {
  ...
  _ResizeMoveResult? _transformUpdate(
    ...
    // ── Swap logic ──────────────────────────────────────────────────────
    bool inSwapZone = false;

    // Maintain active swap: after a swap, A occupies B's old cells so
    // holdItem == id when the cursor is still over the swap area.
    // Keep the swap alive instead of tearing it down every frame.
    if (_swapping && _currentSwapTarget != null && holdItem == id) {
      inSwapZone = true;
    }

    // Detect new swap target (only when not already maintaining a swap)
    if (!inSwapZone &&
        _layoutController.swapOnEditing &&
        holdItem != null &&
        holdItem != id) {
      ...
      if (holdingItem != null) {
        ...
        if (offsetFromHold.dx > 20 && offsetFromHold.dy > 20) {
          ...
          if (notRight && notBottom) {
            ...
            if (!_swapping || _currentSwapTarget != holdItem) {
              // New swap target – undo any previous swap first
              _removeSwap();
              var result = _trySwap(holdItem, holdingItem, newTransform);
              if (result != null) {
                return result;
              }
              // Swap failed (A can't fit at B's position) – fall through
              inSwapZone = false;
            }
            // else: same target as current swap – keep it, update visual only
          }
        }
      }
    }

    if (inSwapZone && _swapping) {
      // Still hovering on the swap target – just track cursor
      if (_transform == null) {
    ...
    // Not in swap zone – undo any active swap
    final reverseDif = _removeSwapAndGetReverseDif();
    if (reverseDif != null && reverseDif != Offset.zero) {
      // Reverse the _moveStartOffset adjustment from the swap
      _onTransformProcess = false;
      return _ResizeMoveResult()
        ..isChanged = true
        ..startDifference = reverseDif;
    }

    // ── Normal placement logic (unchanged) ──────────────────────────────
