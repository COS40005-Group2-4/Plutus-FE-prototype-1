    ...
    return AppCard(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.calendar_view_week, color: t.text, size: 18),
...
class _HeatmapContentState extends State<_HeatmapContent> {
  ...
  /// Maps daily spend intensity onto the 5-step gold heatmap ramp
  /// (bucket 0 = no/lowest spend, bucket 4 = at-or-near peak spend).
  Color _cellColor(DateTime date, PlutusTokens t) {
    final ramp = t.heatmapRamp;
    if (date.isAfter(_today)) return ramp[0];
    final amount = _dailyTotals[date];
    if (amount == null || _peakAmount == 0) return ramp[0];
    final intensity = (amount / _peakAmount).clamp(0.0, 1.0);
    final bucket = (intensity * (ramp.length - 1)).round().clamp(0, ramp.length - 1);
    return ramp[bucket];
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (_selectedDate != null) ...[
          ...
          _buildTooltip(_selectedDate!, t),
        ],
      ],
    );
  }

  Widget _buildGrid(PlutusTokens t) {
    return Expanded(
      child: Column(
        children: [
            ...
            children: [
              ...
              ...List.generate(_kWeeks, (col) {
                return Expanded(
                  child: Text(
                    _columnHeader(col),
                    style: TextStyle(color: t.textMuted, fontSize: 7),
  ...
  Widget _buildTooltip(DateTime date, PlutusTokens t) {
